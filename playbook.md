...
- [3.7 回写流 4 类](#37-回写流-4-类)
- [3.8 紧急 lane · gate:waived 授权机制](#38-紧急-lane--gatewaived-授权机制)
...
| 层 | 范围 | 寿命 | 物理位置 | 角色 |
|---|---|---|---|---|
| **L1 per-feature** | 一个需求 | 短期，feature 完成归档 | `specs/NNN-<feature>/` | SPEC（工作底稿）|
| **L2 per-service** | 一个服务 | 长期，服务存在期 | `arch-kit/docs/services/<svc>/` | 3 强制 + 4 按需 + 1 自动（混合 SSOT/VIEW/AUTO，靠 ROLE 徽章区分）|
| **L3 全局 SSOT** | 跨服务 / 平台级 | 长期 | `contracts/` + `docs/architecture/` + `docs/adr/` + `tech-standards/` + `apps/<svc>/migrations/` | 不变量 / 契约 / 决策 / 标准 / schema |

**核心断言**：L1 寿命短，L2/L3 寿命长；L1 必须**回写** L2/L3（见 [§3.7](#37-回写流-4-类)），否则下一 feature 漂移。

**SSOT 红线**（详 design-philosophy §2）：

1. VIEW 文件**禁止**复制 SSOT 的 schema / 枚举 / 字段定义
2. 全平台需一致的信息**必须**放 `tech-standards/`，不下放到 service VIEW（错误码 / 鉴权矩阵 / 幂等模式 / SLO 等级 / 数据分级 等）
3. `_data-index.md` 头部 `AUTO-GENERATED, DO NOT EDIT`，人改即漂移
...
4.  specs/NNN-<slug>/impact-analysis.md  ← BC×服务×契约×数据 影响地图（T1 产物 · feature 共享）
5.  specs/NNN-<slug>/plan.md             ← HOW 高层（feature 共享）
6.  specs/NNN-<slug>/research.md         ← 技术选型对比（feature 共享）
7.  specs/NNN-<slug>/data-model.md       ← 实体 schema（feature 共享）
8.  specs/NNN-<slug>/contracts/*         ← API / 事件契约（feature 共享 · 机读）
9.  specs/NNN-<slug>/tasks.md  §T-NNN-XX ← 当前 task 行（task 独有）
10. specs/NNN-<slug>/quickstart.md       ← 自验收场景（feature 共享）
```
...
**(a) /speckit.* 5 步链 ↔ 5 件套**（顺序固定 · 每步产出后 PO + TL 必须 Approve 才进下一步）：
...
> 缺一不可入 T4.2（DoR 硬约束）；Spec-Kit 默认产出 `research.md`，对应 Bundle ⑥；T1 产出的 `impact-analysis.md` 占 Bundle ④（非五件套，但同入 context）。
...
| **T4.3** PR review | 多角度审查代码与设计一致性、5 红旗、架构红线 | **`reviewer` subagent**（isolated context · [§3.4](#34-implementer--reviewer-隔离模式) + `ci-prompts/review.md.tmpl`）+ `gate-checklist`（review 模式）| Review 记录 + 必要的 changes requested | Reviewer（5 红旗，AI ≠ approve）|
| **T4.4** QA 验收 | 按 4 象限测试矩阵执行验收测试 | `qa-cases` skill（由专职 QA 驱动）| QA 报告 + 测试结果 | QA |
| **T4.5** 业务验收 | 在准生产/灰度验证业务流是否符合 PO 预期 | — | PO 验收记录 + 触发回写③（服务设计反馈 → `docs/services/<svc>/` 按需文件）| PO + TL |
| **T5** 部署/灰度 | 起草发布方案与回滚方案，按灰度策略上线 | `release-canary` skill 起草 | `release.md` + `rollback.md` + 上线记录 | Release Owner |
| **T6** 观测窗 | 上线后窗口期观测指标/告警/业务异常 | — | 观测报告（指标快照 + 异常清单）| SRE |
| **T7** 复盘 | 用 audit checklist 系统复盘，沉淀经验到 memory | `retro-audit` skill | `retro.md` + Action Items + memory 沉淀 | Eng Lead |
...
> 引自 design-philosophy §5（D18）。
> **层级缩写**：L1 = `specs/NNN-<feature>/`（短期）· L2 = `arch-kit/docs/services/<svc>/`（长期）· L3 = `contracts/` + `docs/architecture/` + `docs/adr/` + `tech-standards/` + `apps/<svc>/migrations/`（全局 SSOT）。

| # | 内容 | 源（L1）| 目 | 时机 | 责任 | 自动度 |
|---|---|---|---|---|---|---|
| ① | 契约汇总 | `specs/NNN/contracts/*.yaml`（`/speckit.plan` 产出）| `contracts/<bctx>/{openapi,asyncapi,proto}/` (L3) | **T2 末** 定稿 / PR merge | 作者 + TL approve；冲突时人审 | **半自动**（CI sync workflow，W16 待建）|
| ② | 数据 DDL | `specs/NNN/data-model.md` 设计转 SQL | `apps/<svc>/migrations/*.sql` (L3) + `docs/services/<svc>/_data-index.md` (L2 AUTO) | **T4.2** 编码同 PR | 作者写 SQL；CI 生 `_data-index.md` | **半自动**（CI build-data-index，W12）|
| ③ | 服务设计反馈 | `specs/NNN/plan.md` 实现细节 | `docs/services/<svc>/{api,events,saga,state-machine,runbook,non-functional,README}.md` (L2) 中**按需开的几个** | **T4.5 后 / T5** | 作者手改；`gate-checklist` 提醒 | 手动（无法机械化）|
| ④ | 架构 / 标准 / 决策 | impact + plan + 实现暴露 | `docs/architecture/` / `docs/adr/` / `tech-standards/` (L3) | T1 草案 → T2 锁定 → T7 收尾 | 架构组 / 标准组 | 手动（横切 R 触发）|

**校验规则**（CI 兜底）：

- 回写①：CI sync workflow（W16）扫 `specs/NNN/contracts/` status=approved 同步到 `contracts/<bctx>/`，冲突 fail；drift checker（W15）反向 diff 兜底
- 回写②：`_data-index.md` 自动 regen（W12）；PR 检查 migrations 改动是否触发 regen
- 回写③：PR template 加 `[ ] 已同步更新 docs/services/<svc>/ 受影响文件`，`gate-checklist` 列受影响服务
- 回写④：由 R 横切触发，无 CI 强制
...
完整 SOP 步骤**不可跳过**，但当某步本 feature 无产物时（例：feature 不涉及架构变更 → T1 无 `impact-analysis.md`），该步对应 Gate 角色可**授权不审**：在 PR / Issue 标 `gate:waived: <step-id>` + 1 行理由。

| 适用场景 | 不适用 |
|---|---|
| 该步骤本身被本 feature 跳过（无产物可审）| 有产物但想绕过审批 |
| Hotfix 紧急流（事后补审 + retro 复审）| 常规 feature |

**审计**：所有 `gate:waived` 进 T7 复盘汇总，季度治理报告统计滥用率。具体授权矩阵后续团队治理迭代优化。
